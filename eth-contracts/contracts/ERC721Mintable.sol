...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused {
        require(_paused == false, 'Contract is currently in a paused state');
        _;
    }

    modifier paused {
        require(_paused == true, 'Contract is currently NOT in a paused state');
        _;
    }
    ...
    function removePause() public onlyOwner paused {
        _paused = false;
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(ownerOfTokenId != to, 'Cannot approve same address to transfer.');

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == ownerOfTokenId || isApprovedForAll(ownerOfTokenId, msg.sender), 'Caller is not authorized');

        // TODO add 'to' address to token approvals
        _tokenApprovals[tokenId] = to;

        // TODO emit Approval Event
        emit Approval(ownerOfTokenId, to, tokenId);
    }
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == address(0), 'This tokenId already exists');
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOfTokenId, 'From address is not the owner of the tokenId');
...
contract BreesERC721Token is ERC721Metadata {
